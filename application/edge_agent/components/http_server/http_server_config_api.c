...
static const config_field_def_t CONFIG_FIELDS[] = {
    ...
    CONFIG_FIELD("wifi",         wifi_proxy_enabled),
    CONFIG_FIELD("wifi",         wifi_proxy_address),
    CONFIG_FIELD("wifi",         wifi_proxy_port),
    ...
    CONFIG_FIELD("obs",          obs_endpoint),
    CONFIG_FIELD("obs",          obs_bucket),
    CONFIG_FIELD("obs",          obs_access_key),
    CONFIG_FIELD("obs",          obs_secret_key),
    CONFIG_FIELD("obs",          obs_security_token),

    CONFIG_FIELD("speech",       xfyun_app_id),
    CONFIG_FIELD("speech",       xfyun_api_key),
    CONFIG_FIELD("speech",       xfyun_api_secret),
...
static bool is_tcp_port_string(const char *value)
{
    char *end = NULL;
    unsigned long port;
...
static bool is_bool_string(const char *value)
{
    return value &&
           (strcmp(value, "0") == 0 ||
            strcmp(value, "1") == 0 ||
...
static bool bool_string_is_true(const char *value)
{
...
static esp_err_t config_post_handler(httpd_req_t *req)
{
    ...
    for (size_t i = 0; i < CONFIG_FIELD_COUNT; i++) {
        ...
        if (strcmp(field->name, "wifi_proxy_enabled") == 0 &&
                !is_bool_string(item->valuestring)) {
            cJSON_Delete(root);
            free(config);
            return httpd_resp_send_err(req,
                                       HTTPD_400_BAD_REQUEST,
                                       "wifi_proxy_enabled must be 0, 1, false, or true");
        }
        strlcpy(field_mutable(config, field), item->valuestring, field->size);
        applied_count++;
    }

    if ((config->wifi_proxy_address[0] && !config->wifi_proxy_port[0]) ||
            (!config->wifi_proxy_address[0] && config->wifi_proxy_port[0])) {
        cJSON_Delete(root);
        free(config);
        return httpd_resp_send_err(req,
                                   HTTPD_400_BAD_REQUEST,
                                   "wifi_proxy_address and wifi_proxy_port must be configured together");
    }
    if (config->wifi_proxy_port[0] && !is_tcp_port_string(config->wifi_proxy_port)) {
        cJSON_Delete(root);
        free(config);
        return httpd_resp_send_err(req,
                                   HTTPD_400_BAD_REQUEST,
                                   "wifi_proxy_port must be an integer between 1 and 65535");
    }
    if (bool_string_is_true(config->wifi_proxy_enabled) &&
            (!config->wifi_proxy_address[0] || !config->wifi_proxy_port[0])) {
        cJSON_Delete(root);
        free(config);
        return httpd_resp_send_err(req,
                                   HTTPD_400_BAD_REQUEST,
                                   "wifi_proxy_address and wifi_proxy_port are required when proxy is enabled");
    }
