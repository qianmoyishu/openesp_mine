...
static void cap_im_wechat_redact_url_for_log(const char *url, char *out, size_t out_size)
{
                                            ...
                                            cap_im_wechat_http_resp_t *response)
{
    ...
    if (status < 200 || status >= 300) {
        char redacted_url[CAP_IM_WECHAT_URL_BUF_SIZE];

        cap_im_wechat_redact_url_for_log(url, redacted_url, sizeof(redacted_url));
        ESP_LOGW(TAG, "WeChat HTTP %s failed: status=%d body=%s", redacted_url, status,
