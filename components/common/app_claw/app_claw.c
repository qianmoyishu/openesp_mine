...
static bool app_config_bool_is_true(const char *value)
{
                             ...
                             const app_claw_storage_paths_t *paths)
{
    claw_memory_config_t memory_config = {
        ...
        .llm = {
            ...
            .proxy_host = app_config_bool_is_true(config->wifi_proxy_enabled) ?
                          config->wifi_proxy_address : NULL,
            .proxy_port = (uint16_t)strtoul(config->wifi_proxy_port, NULL, 10),
            .proxy_enabled = app_config_bool_is_true(config->wifi_proxy_enabled) &&
                             config->wifi_proxy_address[0] &&
                             config->wifi_proxy_port[0],
...
static const char *app_llm_provider_name(const app_claw_config_t *config)
{
    ...
    if (config->llm_base_url[0] &&
        (strstr(config->llm_base_url, "googleapis.com") != NULL || 
         strcmp(config->llm_profile, "gemini") == 0)) {
        return "Gemini";
    }
    if (config->llm_base_url[0] &&
                         ...
                         const app_claw_storage_paths_t *paths)
{
    ...
    core_config.proxy_host = app_config_bool_is_true(config->wifi_proxy_enabled) ?
                             config->wifi_proxy_address : NULL;
    core_config.proxy_port = (uint16_t)strtoul(config->wifi_proxy_port, NULL, 10);
    core_config.proxy_enabled = app_config_bool_is_true(config->wifi_proxy_enabled) &&
                                config->wifi_proxy_address[0] &&
                                config->wifi_proxy_port[0];
