                                        ...
                                        char **out_error_message)
{
    ...
    ctx->proxy_host = config->proxy_host ? strdup(config->proxy_host) : NULL;
    ctx->proxy_port = config->proxy_port;
    ctx->proxy_enabled = config->proxy_enabled && config->proxy_host && config->proxy_host[0] && config->proxy_port;
    ...
    if (!ctx->api_key || !ctx->model || !ctx->base_url || !ctx->auth_type ||
            (config->proxy_host && !ctx->proxy_host)) {
                                        ...
                                        char **out_error_message)
{
    ...
    http_request.proxy_host = ctx->proxy_host;
    http_request.proxy_port = ctx->proxy_port;
    http_request.proxy_enabled = ctx->proxy_enabled;
